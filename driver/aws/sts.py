...
from mypy_boto3_iam.type_defs import EvaluationResultTypeDef, SimulatePolicyResponseTypeDef
...
def get_sts_credentials(role_arn: str,
                        external_id: str,
                        client: STSClient
                        ) -> CredentialsTypeDef:
    """
    ...
    resp = None
    try:
        resp = client.assume_role(RoleArn=role_arn,
    ...
    try:
        ...
        if ErrorMessage.ASSUME_ROLE_NOT_ALLOWED.value in msg:
            raise InvalidPermissionError(msg, ErrorCode.ASSUME_ROLE_NOT_ALLOWED.value) from None
        raise ex
